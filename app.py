...
model_path = 'result/train_2024_05_30_00_22/ZeroGan_Adam_state_epoch_90.pt'
opt = json.load(open(model_path.replace('ZeroGan_Adam_state_epoch_90.pt', 'config.json')))
...
def test_single_image(image, model, image_to_input, discriminator, device, net, patch_size=14):
...
def run_anomaly_detection(image):
    try:
        original_image, reconstructed_image, difference_map, anomaly_score = test_single_image(image, model, image_to_input, discriminator, device, net)
